...
from jarvishep.observable_io import resolve_schema_path  # noqa: E402
...
class ObservableSchemaIOTests(unittest.TestCase):
    ...
    def test_add_data_supports_array_and_list_and_creates_schema(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(schema["columns"]["arr"]["flatten"]["mode"], "json")
            self.assertEqual(schema["columns"]["lst"]["flatten"]["mode"], "json")
    ...
    def test_invalid_modes_are_normalized_and_persisted(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(repaired["flatten_defaults"]["array"], "json")
            self.assertEqual(repaired["columns"]["arr"]["flatten"]["mode"], "json")
