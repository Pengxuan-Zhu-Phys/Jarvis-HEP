...
def _assert_max_width(block: str, max_width: int):
    for line in block.splitlines():
        assert len(line) <= max_width, f"line exceeds max width {max_width}: {line!r}"


def test_two_column_log_wraps_long_values_under_80_columns():
    long_value = "x" * 160
...
def test_two_column_log_wraps_long_keys_under_80_columns():
...
def test_two_column_log_wraps_title_under_80_columns():
