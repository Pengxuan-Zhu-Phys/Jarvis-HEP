...
************************************************************************
      ...
      INTEGER NFAIL(NFL),IFAIL,I,TOT,ITOT,NTOT,IDUM,ISEED,LOOP
      INTEGER ITER,ITERMU,Q2FIX,OMGFLAG,MAFLAG,MOFLAG,GMUFLAG,HFLAG
      INTEGER TOTMIN,TOTMAX,NMAX,IP,JM,JL,ITRY,NMSFLAG,GMFLAG,SFFLAG
      INTEGER NFLAG,UNCERTFLAG,GRFLAG,CFLAG(5)
      ...
      DOUBLE PRECISION MSUSYEFF,MMESS,N5,GAU,DETM,SIGMU,Q2,Q2MIN
      ...
      DOUBLE PRECISION LPPMEM,LTTMEM,PARMEM(25),DELMB,DELML,DEL1
      DOUBLE PRECISION MUINIT,MUSTEP,MUFAIL
...
*   Beginning of the random scan
 ...
 101    MSUSYEFF=MSUSYEFFCEN
     .  +MAX(DABS(MSUSYEFFCEN),MSUSYEFFMIN)*MSUSYEFFDEV*GAU(IDUM)
        IF(MSUSYEFF.LE.MZ)GOTO 101
       ENDIF

       IF(MMESSDEV.EQ.0d0)THEN
        MMESS=MMESSCEN
       ELSE
 102    MMESS=MMESSCEN*MAX(MMESSCEN,MMESSMIN)**(MMESSDEV*GAU(IDUM))
        IF(MMESS.GE.1d15)GOTO 102
        IF(MMESS.LE.1d1*MSUSYEFF)GOTO 102
       ENDIF

       IF(TBDEV.EQ.0d0)THEN
        PAR(3)=TBCEN
       ELSE
 103    PAR(3)=TBCEN+MAX(TBCEN,TBMIN)*TBDEV*GAU(IDUM)
       ...
       IF(XIUDEV.EQ.0d0)THEN
        XIU=XIUCEN
       ELSE
        XIU=XIUCEN+MAX(DABS(XIUCEN),XIUMIN)*XIUDEV*GAU(IDUM)
       ENDIF
       ...
       IF(GMFLAG.EQ.2 .OR. GMFLAG.EQ.4)THEN
        IF(LPPDEV.EQ.0d0)THEN
         LPPMES=LPPCEN
        ELSE
         LPPMES=LPPCEN+MAX(DABS(LPPCEN),LPPMIN)*LPPDEV*GAU(IDUM)
        ENDIF
        IF(LTTDEV.EQ.0d0)THEN
         LTTMES=LTTCEN
        ELSE
         LTTMES=LTTCEN+MAX(DABS(LTTCEN),LTTMIN)*LTTDEV*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       IF(LUDEV.EQ.0d0)THEN
        LUMES=LUCEN
       ELSE
        LUMES=LUCEN+MAX(DABS(LUCEN),LUMIN)*LUDEV*GAU(IDUM)
       ENDIF

       IF(LDDEV.EQ.0d0)THEN
        LDMES=LDCEN
       ELSE
        LDMES=LDCEN+MAX(DABS(LDCEN),LDMIN)*LDDEV*GAU(IDUM)
       ENDIF

       IF(LTDEV.EQ.0d0)THEN
        LTMES=LTCEN
       ELSE
        LTMES=LTCEN+MAX(DABS(LTCEN),LTMIN)*LTDEV*GAU(IDUM)
       ENDIF

       IF(LBDEV.EQ.0d0)THEN
        LBMES=LBCEN
       ELSE
        LBMES=LBCEN+MAX(DABS(LBCEN),LBMIN)*LBDEV*GAU(IDUM)
       ENDIF

       IF(LLDEV.EQ.0d0)THEN
        LLMES=LLCEN
       ELSE
        LLMES=LLCEN+MAX(DABS(LLCEN),LLMIN)*LLDEV*GAU(IDUM)
       ENDIF
...
!      ENDIF
      SFFLAG=0
      ITRY=1
      IF(IFAIL.NE.0 .AND. ITOT.EQ.1)THEN
...
*******************************************************************
      ...
      INTEGER NTOT,ISEED,GMFLAG,N0,NLOOP,NBER,NPAR,ERR,VFLAG,NMSFLAG
      INTEGER OMGFLAG,MAFLAG,MOFLAG,PFLAG,GMUFLAG,HFLAG,Z3FLAG
      INTEGER NFLAG,GRFLAG,CFLAG(5)
...
*   READ EXTPAR
      ELSEIF(CHBLCK(1:6).EQ.'EXTPAR')THEN
       ...
       IF(IX.EQ.73) XIUCEN=VAL
       IF(IX.EQ.736) XIUDEV=VAL
       IF(IX.EQ.737) XIUMIN=VAL
       IF(IX.EQ.74) LPPCEN=VAL
       IF(IX.EQ.746) LPPDEV=VAL
       IF(IX.EQ.747) LPPMIN=VAL
       IF(IX.EQ.75) LTTCEN=VAL
       IF(IX.EQ.756) LTTDEV=VAL
       IF(IX.EQ.757) LTTMIN=VAL
       IF(IX.EQ.76) LUCEN=VAL
       IF(IX.EQ.766) LUDEV=VAL
       IF(IX.EQ.767) LUMIN=VAL
       IF(IX.EQ.77) LDCEN=VAL
       IF(IX.EQ.776) LDDEV=VAL
       IF(IX.EQ.777) LDMIN=VAL
       IF(IX.EQ.78) LTCEN=VAL
       IF(IX.EQ.786) LTDEV=VAL
       IF(IX.EQ.787) LTMIN=VAL
       IF(IX.EQ.79) LBCEN=VAL
       IF(IX.EQ.796) LBDEV=VAL
       IF(IX.EQ.797) LBMIN=VAL
       IF(IX.EQ.80) LLCEN=VAL
       IF(IX.EQ.806) LLDEV=VAL
...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(MOFLAG.LT.0 .OR. MOFLAG.GT.7)THEN
       ...
       ERR=1
      ENDIF
      IF(HFLAG.LT.0 .OR. HFLAG.GT.2)THEN
       WRITE(0,1)"HFLAG MUST BE IN [0-2]"
       ERR=1
      ENDIF
      IF(NFLAG.LT.0 .OR. NFLAG.GT.3)THEN
       WRITE(0,1)"NFLAG MUST BE IN [0-3]"
       ERR=1
      ENDIF
      ...
      IF(XISCEN.NE.1d99 .AND. MSCEN.NE.1d99)THEN
       ...
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. KCEN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => KCEN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. ALCEN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => ALCEN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. MSCEN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => MSCEN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. XIFCEN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => XIFCEN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. XISCEN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => XISCEN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      ...
      IF(GMFLAG.NE.0 .AND. DMIN.EQ.1d99)THEN
       ...
       ERR=1
      ENDIF
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.3 .AND. (XIUCEN.NE.0d0
     ..OR. XIUDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=/=1,3 => XIU CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.2 .AND. GMFLAG.NE.4 .AND. (LPPCEN.NE.0d0
     ..OR. LPPDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=/=2,4 => LPP CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.2 .AND. GMFLAG.NE.4 .AND. (LTTCEN.NE.0d0
     ..OR. LTTDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=/=2,4 => LTT CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.2 .AND. (LUCEN.NE.0d0
     ..OR. LUDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=/=1,2 => LU CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.2 .AND. (LDCEN.NE.0d0
     ..OR. LDDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=/=1,2 => LD CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.EQ.0 .AND. (LTCEN.NE.0d0 .OR. LTDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=0 => LT CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.EQ.0 .AND. (LBCEN.NE.0d0 .OR. LBDEV.NE.1d99))THEN
       WRITE(0,1)"GMFLAG=0 => LB CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.2 .AND. (LLCEN.NE.0d0
     ..OR. LLDEV.NE.1d99))THEN
...
*********************************************************************
      ...
      INTEGER I,NRES,IRES,NBIN,GMFLAG,GRFLAG
      INTEGER NSUSY,NGUT,NMES,IMAX,IFAIL
     ...
     . MQMES,MUMES,MDMES,ML3MES,ME3MES,MLMES,MEMES
      ...
      DOUBLE PRECISION OMG,OMGMIN,OMGMAX,DELMB,DELML,DEL1
