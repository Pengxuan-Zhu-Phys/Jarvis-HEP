...
************************************************************************
      ...
      INTEGER NTOT,IDUM,NM1,NM2,NM3,NTB,NL,NK,NAL,NAK,NMU
      INTEGER NXIF,NXIS,NMUP,NMSP,NM3H,NMA,NMP,NAQ,NMQ,NM1I
      INTEGER NM2I,NM3I,NAU3I,NAD3I,NAE3I,NLI,NKI,NXIFI,NXISI
      INTEGER NMUPI,NMSPI,NM3HI,CFLAG(5)
      ...
      DOUBLE PRECISION PAR(NPAR),PROB(NPROB),RAN2,Q2,PI
     ...
     . MSFLAG,AKFLAG,ALFLAG,XISIFLAG
      COMMON/STEPS/NTOT,IDUM,NM1,NM2,NM3,NTB,NL,NK,NAL,NAK,NMU,
     . NMUP,NMSP,NM3H,NMA,NMP,NAQ,NMQ,NM1I,NXIF,NXIS,NM2I,NM3I,
     . NAU3I,NAD3I,NAE3I,NLI,NKI,NXIFI,NXISI,NMUPI,NMSPI,NM3HI
...
*   Beginning of the scan
      ...
      ELSE
       IF(NM2.EQ.0)THEN
        REALP(4)=M2MIN+(M2MAX-M2MIN)*RAN2(IDUM)
       ELSE
        REALP(4)=M2MIN*(M2MAX/M2MIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NM1.EQ.0)THEN
        REALP(3)=M1MIN+(M1MAX-M1MIN)*RAN2(IDUM)
       ELSE
        REALP(3)=M1MIN*(M1MAX/M1MIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NM3.EQ.0)THEN
        REALP(5)=M3MIN+(M3MAX-M3MIN)*RAN2(IDUM)
       ELSE
        REALP(5)=M3MIN*(M3MAX/M3MIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NL.EQ.0)THEN
        REALP(1)=LMIN+(LMAX-LMIN)*RAN2(IDUM)
       ELSE
        REALP(1)=LMIN*(LMAX/LMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NK.EQ.0)THEN
        REALP(2)=KMIN+(KMAX-KMIN)*RAN2(IDUM)
       ELSE
        REALP(2)=KMIN*(KMAX/KMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NMU.EQ.0)THEN
        REALP(14)=MUMIN+(MUMAX-MUMIN)*RAN2(IDUM)
       ELSE
        REALP(14)=MUMIN*(MUMAX/MUMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      IF(MOD(MAFLAG,3).EQ.0)THEN
       ...
       ELSE
        IF(NXIF.EQ.0)THEN
         REALP(9)=XIFMIN+(XIFMAX-XIFMIN)*RAN2(IDUM)
        ELSE
         REALP(9)=XIFMIN*(XIFMAX/XIFMIN)**RAN2(IDUM)
        ENDIF
       ENDIF
      ...
      IF(MAFLAG/3.EQ.0)THEN
       ...
       ELSE
        IF(NXIS.EQ.0)THEN
         REALP(10)=XISMIN+(XISMAX-XISMIN)*RAN2(IDUM)
        ELSE
         REALP(10)=XISMIN*(XISMAX/XISMIN)**RAN2(IDUM)
        ENDIF
       ENDIF
      ...
      ELSE
       IF(NMUP.EQ.0)THEN
        REALP(11)=MUPMIN+(MUPMAX-MUPMIN)*RAN2(IDUM)
       ELSE
        REALP(11)=MUPMIN*(MUPMAX/MUPMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NMSP.EQ.0)THEN
        REALP(12)=MSPMIN+(MSPMAX-MSPMIN)*RAN2(IDUM)
       ELSE
        REALP(12)=MSPMIN*(MSPMAX/MSPMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      ELSE
       IF(NM3H.EQ.0)THEN
        REALP(13)=M3HMIN+(M3HMAX-M3HMIN)*RAN2(IDUM)
       ELSE
        REALP(13)=M3HMIN*(M3HMAX/M3HMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AQMIN.EQ.AQMAX)THEN
       REALP(6)=AQMIN
      ELSE
       IF(NAQ.EQ.0)THEN
        REALP(6)=AQMIN+(AQMAX-AQMIN)*RAN2(IDUM)
       ELSE
        REALP(6)=AQMIN*(AQMAX/AQMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(MQMIN.EQ.MQMAX)THEN
       PAR(7)=MQMIN**2
      ELSE
       IF(NMQ.EQ.0)THEN
        PAR(7)=(MQMIN+(MQMAX-MQMIN)*RAN2(IDUM))**2
       ELSE
        PAR(7)=(MQMIN*(MQMAX/MQMIN)**RAN2(IDUM))**2
       ENDIF
      ENDIF

      IF(LIMIN.EQ.LIMAX)THEN
       IMAGP(1)=LIMIN
      ELSE
       IF(NLI.EQ.0)THEN
        IMAGP(1)=LIMIN+(LIMAX-LIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(1)=LIMIN*(LIMAX/LIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(KIMIN.EQ.KIMAX)THEN
       IMAGP(2)=KIMIN
      ELSE
       IF(NKI.EQ.0)THEN
        IMAGP(2)=KIMIN+(KIMAX-KIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(2)=KIMIN*(KIMAX/KIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(M1IMIN.EQ.M1IMAX)THEN
       IMAGP(3)=M1IMIN
      ELSE
       IF(NM1I.EQ.0)THEN
        IMAGP(3)=M1IMIN+(M1IMAX-M1IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(3)=M1IMIN*(M1IMAX/M1IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(M2IMIN.EQ.M2IMAX)THEN
       IMAGP(4)=M2IMIN
      ELSE
       IF(NM2I.EQ.0)THEN
        IMAGP(4)=M2IMIN+(M2IMAX-M2IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(4)=M2IMIN*(M2IMAX/M2IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(M3IMIN.EQ.M3IMAX)THEN
       IMAGP(5)=M3IMIN
      ELSE
       IF(NM3I.EQ.0)THEN
        IMAGP(5)=M3IMIN+(M3IMAX-M3IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(5)=M3IMIN*(M3IMAX/M3IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AU3IMIN.EQ.AU3IMAX)THEN
       IMAGP(6)=AU3IMIN
      ELSE
       IF(NAU3I.EQ.0)THEN
        IMAGP(6)=AU3IMIN+(AU3IMAX-AU3IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(6)=AU3IMIN*(AU3IMAX/AU3IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AD3IMIN.EQ.AD3IMAX)THEN
       IMAGP(7)=AD3IMIN
      ELSE
       IF(NAD3I.EQ.0)THEN
        IMAGP(7)=AD3IMIN+(AD3IMAX-AD3IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(7)=AD3IMIN*(AD3IMAX/AD3IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(AE3IMIN.EQ.AE3IMAX)THEN
       IMAGP(8)=AE3IMIN
      ELSE
       IF(NAE3I.EQ.0)THEN
        IMAGP(8)=AE3IMIN+(AE3IMAX-AE3IMIN)*RAN2(IDUM)
       ELSE
        IMAGP(8)=AE3IMIN*(AE3IMAX/AE3IMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(XIFIMIN.EQ.XIFIMAX)THEN
       IMAGP(9)=XIFIMIN
      ELSE
       IF(NXIFI.EQ.0)THEN
        IMAGP(9)=XIFIMIN+(XIFIMAX-XIFIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(9)=XIFIMIN*(XIFIMAX/XIFIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(XISIMIN.EQ.XISIMAX)THEN
       IMAGP(10)=XISIMIN
      ELSE
       IF(NXISI.EQ.0)THEN
        IMAGP(10)=XISIMIN+(XISIMAX-XISIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(10)=XISIMIN*(XISIMAX/XISIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(MUPIMIN.EQ.MUPIMAX)THEN
       IMAGP(11)=MUPIMIN
      ELSE
       IF(NMUPI.EQ.0)THEN
        IMAGP(11)=MUPIMIN+(MUPIMAX-MUPIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(11)=MUPIMIN*(MUPIMAX/MUPIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(MSPIMIN.EQ.MSPIMAX)THEN
       IMAGP(12)=MSPIMIN
      ELSE
       IF(NMSPI.EQ.0)THEN
        IMAGP(12)=MSPIMIN+(MSPIMAX-MSPIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(12)=MSPIMIN*(MSPIMAX/MSPIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF

      IF(M3HIMIN.EQ.M3HIMAX)THEN
       IMAGP(13)=M3HIMIN
      ELSE
       IF(NM3HI.EQ.0)THEN
        IMAGP(13)=M3HIMIN+(M3HIMAX-M3HIMIN)*RAN2(IDUM)
       ELSE
        IMAGP(13)=M3HIMIN*(M3HIMAX/M3HIMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
...
*******************************************************************
      ...
      INTEGER NTOT,ISEED,NM1,NM2,NM3,NTB,NL,NK,NAL,NAK,NMU
      INTEGER NXIF,NXIS,NMUP,NMSP,NM3H,NMA,NMP,NAQ,NMQ,NM1I
      INTEGER NM2I,NM3I,NAU3I,NAD3I,NAE3I,NLI,NKI,NXIFI,NXISI
      INTEGER NMUPI,NMSPI,NM3HI,CFLAG(5)
     ...
     . LIMAX,KIMIN,KIMAX,XIFIMIN,XIFIMAX,XISIMIN,XISIMAX,MUPIMIN,
     . MUPIMAX,MSPIMIN,MSPIMAX,M3HIMIN,M3HIMAX
      COMMON/STEPS/NTOT,ISEED,NM1,NM2,NM3,NTB,NL,NK,NAL,NAK,NMU,
     . NMUP,NMSP,NM3H,NMA,NMP,NAQ,NMQ,NM1I,NXIF,NXIS,NM2I,NM3I,
     . NAU3I,NAD3I,NAE3I,NLI,NKI,NXIFI,NXISI,NMUPI,NMSPI,NM3HI
      COMMON/SCANFLAGS/M1FLAG,M2FLAG,M3FLAG,MHDFLAG,MHUFLAG,
     . MSFLAG,AKFLAG,ALFLAG,XISIFLAG
...
*   Total number of points
      ...
      IF(NTOT.LE.0 .OR. (NM1.NE.0 .AND. NM1.NE.1) .OR.
     .(NM2.NE.0 .AND. NM2.NE.1) .OR. (NM3.NE.0 .AND. NM3.NE.1) .OR.
     .(NTB.NE.0 .AND. NTB.NE.1) .OR. (NL.NE.0 .AND. NL.NE.1) .OR.
     .(NK.NE.0 .AND. NK.NE.1) .OR. (NAL.NE.0 .AND. NAL.NE.1) .OR.
     .(NAK.NE.0 .AND. NAK.NE.1) .OR. (NMU.NE.0 .AND. NMU.NE.1) .OR.
     .(NXIF.NE.0 .AND. NXIF.NE.1) .OR. (NXIS.NE.0 .AND. NXIS.NE.1).OR.
     .(NMUP.NE.0 .AND. NMUP.NE.1) .OR. (NMSP.NE.0 .AND. NMSP.NE.1) .OR.
     .(NM3H.NE.0 .AND. NM3H.NE.1) .OR. (NMA.NE.0 .AND. NMA.NE.1) .OR.
     .(NMP.NE.0 .AND. NMP.NE.1) .OR. (NAQ.NE.0 .AND. NAQ.NE.1) .OR.
     .(NMQ.NE.0 .AND. NMQ.NE.1) .OR. (NLI.NE.0 .AND. NLI.NE.1) .OR.
     .(NKI.NE.0 .AND. NKI.NE.1) .OR. (NM1I.NE.0 .AND. NM1I.NE.1) .OR.
     .(NM2I.NE.0 .AND. NM2I.NE.1) .OR. (NM3I.NE.0 .AND. NM3I.NE.1) .OR.
     .(NAU3I.NE.0 .AND. NAU3I.NE.1) .OR.
     .(NAD3I.NE.0 .AND. NAD3I.NE.1) .OR.
     .(NAE3I.NE.0 .AND. NAE3I.NE.1) .OR.
     .(NXIFI.NE.0 .AND. NXIFI.NE.1) .OR.
     .(NXISI.NE.0 .AND. NXISI.NE.1) .OR.
     .(NMUPI.NE.0 .AND. NMUPI.NE.1) .OR.
     .(NMSPI.NE.0 .AND. NMSPI.NE.1) .OR.
     .(NM3HI.NE.0 .AND. NM3HI.NE.1))THEN
