...
************************************************************************
      ...
      INTEGER ITER,ITERMU,Q2FIX,OMGFLAG,MAFLAG,MOFLAG
      INTEGER JM,JL,LOOP,NMSFLAG,GMUFLAG,HFLAG,SFFLAG,ITRY
      INTEGER NMSUSYEFF,NMMESS,NTB,NL,NAL,NMUP,NMSP,NXIU
      INTEGER NLPP,NLTT,NLU,NLD,NLT,NLB,NLL,NK,NXIF,NXIS,NMS
      ...
      DOUBLE PRECISION MSUSYEFF,MMESS,N5,RAN2,DETM,SIGMU,Q2,Q2MIN
      DOUBLE PRECISION ALSMZ,ALEMMZ,GF,g1,g2,S2TW,MSREF,D,DMIN
      DOUBLE PRECISION PI,M32,CGR,MPL,MMIN,MMAX
     ...
     . LDN,LDNN,LTN,LTNN,LBN,LBNN,LLN,LLNN,MUN,MUNN
      COMMON/STEPS/NTOT,IDUM,NMSUSYEFF,NMMESS,NTB,NL,NAL,NMUP,
     . NMSP,NXIU,NLPP,NLTT,NLU,NLD,NLT,NLB,NLL,NK,NXIF,NXIS,NMS
...
*   Beginning of the random scan
      ...
      MMIN=MAX(MMESSMIN,10d0*MSUSYEFF)
      MMAX=MIN(MMESSMAX,1d15)

      IF(MMIN.EQ.MMAX)THEN
       MMESS=MMIN
      ELSE
       IF(NMMESS.EQ.0)THEN
        MMESS=MMIN+(MMAX-MMIN)*RAN2(IDUM)
       ELSE
        MMESS=MMIN*(MMAX/MMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
      ...
      IF(XIUMIN.EQ.XIUMAX)THEN
       XIU=XIUMIN
      ELSE
       IF(NXIU.EQ.0)THEN
        XIU=XIUMIN+(XIUMAX-XIUMIN)*RAN2(IDUM)
       ELSE
        XIU=XIUMIN*(XIUMAX/XIUMIN)**RAN2(IDUM)
       ENDIF
      ENDIF
...
*******************************************************************
      ...
      INTEGER I,NLINE,INL,ICH,IX,IVAL,Q2FIX,NMSFLAG,Z3FLAG
      INTEGER NTOT,ISEED,GMFLAG,N0,NLOOP,NBER,NPAR,ERR
      INTEGER NMSUSYEFF,NMMESS,NTB,NL,NAL,NMUP,NMSP,NXIU
      INTEGER NLPP,NLTT,NLU,NLD,NLT,NLB,NLL,NK,NXIF,NXIS,NMS
      INTEGER OMGFLAG,MAFLAG,MOFLAG,PFLAG,GMUFLAG,HFLAG,VFLAG
     ...
     . MSMIN,MSMAX,XIUMIN,XIUMAX,LPPMIN,LPPMAX,LTTMIN,LTTMAX,LUMIN,
     . LUMAX,LDMIN,LDMAX,LTMIN,LTMAX,LBMIN,LBMAX,LLMIN,LLMAX
      COMMON/STEPS/NTOT,ISEED,NMSUSYEFF,NMMESS,NTB,NL,NAL,NMUP,
     . NMSP,NXIU,NLPP,NLTT,NLU,NLD,NLT,NLB,NLL,NK,NXIF,NXIS,NMS
      COMMON/MESCAL/MSUSYEFF,MMESS,N5
      COMMON/FLAGS/OMGFLAG,MAFLAG,MOFLAG
      COMMON/PFLAG/PFLAG
...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(GMFLAG.NE.0 .AND. KMIN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => KMIN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. ALMIN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => ALMIN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. MSMIN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => MSMIN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. XIFMIN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => XIFMIN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.0 .AND. XISMIN.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => XISMIN CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
...
*   Total number of points
      ...
      IF(NTOT.LE.0.OR.(NMSUSYEFF.NE.0 .AND. NMSUSYEFF.NE.1)
     ..OR.(NMMESS.NE.0 .AND. NMMESS.NE.1).OR.(NTB.NE.0 .AND. NTB.NE.1)
     ..OR.(NL.NE.0 .AND. NL.NE.1).OR.(NK.NE.0 .AND. NK.NE.1)
     ..OR.(NAL.NE.0 .AND. NAL.NE.1).OR.(NXIF.NE.0 .AND. NXIF.NE.1)
     ..OR.(NXIS.NE.0 .AND. NXIS.NE.1).OR.(NMUP.NE.0 .AND. NMUP.NE.1)
     ..OR.(NMSP.NE.0 .AND. NMSP.NE.1).OR.(NMS.NE.0 .AND. NMS.NE.1)
     ..OR.(NXIU.NE.0 .AND. NXIU.NE.1).OR.(NLPP.NE.0 .AND. NLPP.NE.1)
     ..OR.(NLTT.NE.0 .AND. NLTT.NE.1).OR.(NLU.NE.0 .AND. NLU.NE.1)
     ..OR.(NLD.NE.0 .AND. NLD.NE.1).OR.(NLT.NE.0 .AND. NLT.NE.1)
     ..OR.(NLB.NE.0 .AND. NLB.NE.1).OR.(NLL.NE.0 .AND. NLL.NE.1))THEN
