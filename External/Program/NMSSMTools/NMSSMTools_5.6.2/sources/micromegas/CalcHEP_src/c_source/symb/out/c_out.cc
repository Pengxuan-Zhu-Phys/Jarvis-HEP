...
static int subproc_sq_not_zero=0;
...
static int   nden_w, nden_s, nden_t, nden_0; /* from writesubprocess */
...
static void  writesubprocess(int nsub,int nsub1,long firstDiag,int totDiag,int* breaker)
{  denlist    den_;
   long      i;
   ...
   fd=fopen(fd_name,"wb"); 
   mark_(&mem_start);
   denominatorStatistic(nsub1, &nden_s, &nden_t, &nden_0, &den_, fd); 
...
static void  make_pinf(void)
{
   ...
   writeF(" static char *names[%d][%d] ={\n",subproc_sq_not_zero,nin + nout);
   for(inftmp = inf, first=1; inftmp ; inftmp = inftmp->next) 
   {  if(first)  {writeF("{"); first=0;} else writeF(",\n{");
...
{  int nden_s,nden_t,nden_0;
   ...
   writeF(" switch(nsub){\n");
   inftmp = inf; 
   for (nsub_=1,inftmp = inf; inftmp; inftmp = inftmp->next,nsub_++)
   {  int n=1;
      marktp mem_start; 
      denlist    den_;
      writeF(" case %d: switch(n){",nsub_);
      mark_(&mem_start);
      denominatorStatistic(inftmp->nsub, &nden_s, &nden_t, &nden_0, &den_, NULL); 
...
static void  make_infbasis(void)
{
   ...
   nsub_=0;
   if(!noCChain)
   for (inftmp = inf; inftmp; inftmp = inftmp->next,nsub_++) 
   {  
      writeF("   case %d : ",nsub_);
...
static int c_prog_int(void)
{
  ...
  /* ======= End of Initialisation ====== */
   ...
   geninf("nprc_ext",subproc_sq_not_zero);
   make_pinf();
   ...
   { 
      ...
      forceUG, pathtocalchep,nvars, nfunc, nin,nout,subproc_sq_not_zero);
   ...
   writeF("extern DNN ");
   for(i=1;i<subproc_sq_not_zero;i++)  writeF("S%d_ext,",i); 
   writeF("S%d_ext;\n",subproc_sq_not_zero); 
   
   writeF("static  DNN * darr[%d]={",subproc_sq_not_zero);
   for(i=1;i<subproc_sq_not_zero;i++)  writeF("&S%d_ext,",i);
   writeF("&S%d_ext};\n",subproc_sq_not_zero);
   ...
   diagrcount = 0;
   init_stat();
   for (nsub=1,dfirst=1,inftmp = inf; inftmp; inftmp=inftmp->next,nsub++)
   {  int colors[MAXINOUT];
      ...
      writesubprocess(nsub,inftmp->nsub,dfirst,inftmp->tot, &breaker);
      dfirst+=inftmp->tot;
      if (breaker) goto exi; 
      if(cBasisPower)
      {
