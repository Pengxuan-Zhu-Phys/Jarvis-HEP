...
************************************************************************
      ...
      INTEGER IFAIL,I,ITER,ITERMU,Q2FIX,LOOP,JM,JL,ITRY,UNCERTFLAG
      INTEGER OMGFLAG,MAFLAG,MOFLAG,GMFLAG,NMSFLAG,SFFLAG,GRFLAG
      INTEGER CFLAG(5)

      DOUBLE PRECISION PAR(NPAR),PROB(NPROB),CHECK,MESTEST,GUTEST
      DOUBLE PRECISION SIGMU,Q2,Q2MIN,MSUSYEFF,MMESS,N5,M32,CGR,MPL
      DOUBLE PRECISION XIFMES,XISMES,MSMES,MUPMES,MSPMES,M3HMES
      DOUBLE PRECISION G1MES,G2MES,G3MES,LMES,KMES,HTMES,HBMES,HLMES
      DOUBLE PRECISION MSREF,D,DMIN,DETM,DELMB,DELML,DEL1
...
*******************************************************************
      ...
      INTEGER N0,NLOOP,NBER,NPAR,OUTFLAG,GRFLAG,CFLAG(5)
...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(GMFLAG.NE.0 .AND. ALINP.NE.1d99)THEN
       WRITE(0,1)"GMFLAG=/=0 => AL CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      ...
      IF(GMFLAG.NE.0 .AND. DMIN.EQ.1d99)THEN
       ...
       ERR=1
      ENDIF
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.3 .AND. XIU.NE.0d0)THEN
       WRITE(0,1)"GMFLAG=/=1,3 => XIU CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.2 .AND. GMFLAG.NE.4 .AND. LPPMES.NE.0d0)THEN
       WRITE(0,1)"GMFLAG=/=2,4 => LPP CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.NE.2 .AND. GMFLAG.NE.4 .AND. LTTMES.NE.0d0)THEN
      ...
      IF(GMFLAG.NE.1 .AND. GMFLAG.NE.2 .AND. LDMES.NE.0d0)THEN
       ...
       ERR=1
      ENDIF
      IF(GMFLAG.EQ.0 .AND. LTMES.NE.0d0)THEN
       WRITE(0,1)"GMFLAG=0 => LT CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(GMFLAG.EQ.0 .AND. LBMES.NE.0d0)THEN
...
*********************************************************************
      ...
      INTEGER I,IFAIL,NSUSY,NGUT,NMES,OUTFLAG,GRFLAG,NBIN
     ...
     . WRITE(17,900) 4,"# MSMES=/=MSREF"
      ...
      WRITE(17,901) 2,MMESS,"MMES"
      WRITE(17,901) 3,TANB,"TANB at MZ"
      WRITE(17,901) 4,SIGMU,"SIGMU"
      IF(N5.NE.1d0)WRITE(17,901) 5,N5,"N5"

      WRITE(17,899) "BLOCK EXTPAR"
      WRITE(17,901) 61,PAR(1),"LAMBDA at MSUSY"
      IF(MAFLAG.EQ.-3 .OR. MAFLAG.EQ.-4)THEN
       WRITE(17,901) 62,PAR(2),"KAPPA at MSUSY"
      ELSE
       WRITE(17,920) 62,PAR(2),"KAPPA at MSUSY"
      ENDIF
      IF(GMFLAG.EQ.0)THEN
       WRITE(17,901) 63,ALINP,"ALAMBDA at MMES"
      ELSE
       WRITE(17,920) 63,ALINP,"ALAMBDA at MMES"
      ENDIF
      WRITE(17,920) 65,PAR(4),"MUEFF at MSUSY"
      IF(MAFLAG.EQ.-1 .OR. MAFLAG.EQ.-2)THEN
       IF(XIFINP.NE.0d0)
     .  WRITE(17,901) 66,XIFINP,"XIF at MMES"
      ELSE
       WRITE(17,920) 66,XIFMES,"XIF at MMES"
      ENDIF
      IF(MAFLAG.EQ.-1 .OR. MAFLAG.EQ.-3)THEN
       IF(XISINP.NE.0d0)
     .  WRITE(17,901) 67,XISINP,"XIS at MMES"
      ELSE
       WRITE(17,920) 67,XISMES,"XIS at MMES"
      ENDIF
      IF(MUPINP.NE.0d0)
     .  WRITE(17,901) 68,MUPINP,"MUP at MMES "
      IF(MSPINP.NE.0d0)
     .  WRITE(17,901) 69,MSPINP,"MSP at MMES "
      IF(MAFLAG.EQ.-2 .OR. MAFLAG.EQ.-4)THEN
       WRITE(17,901) 70,MSINP,"MS at MMES"
      ELSE
       WRITE(17,920) 70,MSMES,"MS at MMES"
      ENDIF
      IF(GMFLAG.NE.0)
     . WRITE(17,920) 71,MSREF,"MS_REF"
      IF(GMFLAG.NE.0)
     . WRITE(17,920) 72,D,"DELTA"
      IF((GMFLAG.EQ.1 .OR. GMFLAG.EQ.3) .AND. XIU.NE.0d0)
     .  WRITE(17,901) 73,XIU,"XiU at MGUT"
      IF((GMFLAG.EQ.2 .OR. GMFLAG.EQ.4) .AND. LPPMES.NE.0d0)
     .  WRITE(17,901) 74,LPPMES,"LPP at MMES"
      IF((GMFLAG.EQ.2 .OR. GMFLAG.EQ.4) .AND. LTTMES.NE.0d0)
     .  WRITE(17,901) 75,LTTMES,"LTT at MMES"
      IF((GMFLAG.EQ.1 .OR. GMFLAG.EQ.2) .AND. LUMES.NE.0d0)
     .  WRITE(17,901) 76,LUMES,"LU at MMES"
      IF((GMFLAG.EQ.1 .OR. GMFLAG.EQ.2) .AND. LDMES.NE.0d0)
     .  WRITE(17,901) 77,LDMES,"LD at MMES"
      IF(GMFLAG.NE.0 .AND. LTMES.NE.0d0)
     .  WRITE(17,901) 78,LTMES,"LT at MMES"
      IF(GMFLAG.NE.0 .AND. LBMES.NE.0d0)
     .  WRITE(17,901) 79,LBMES,"LB at MMES"
      IF((GMFLAG.EQ.1 .OR. GMFLAG.EQ.2) .AND. LLMES.NE.0d0)
     .  WRITE(17,901) 80,LLMES,"LL at MMES"
...
*  From IF(OUTFLAG.NE.1)
     ...
     .        "HTAU(MMES,DR_bar)"
      ...
      WRITE(17,901) 8,MUPMES,"MUP"
      WRITE(17,901) 9,MSPMES,"MSP"
      WRITE(17,901) 10,MSMES,"MS^2"
      WRITE(17,901) 11,M3HMES,"M3H"
     ...
     .        "g1(MLANDAU), (GUT normalization)"
      ...
      WRITE(17,901) 11,FTMES(11),"PM=MMES"
      WRITE(17,901) 12,FTMES(12),"PM=LPP"
      WRITE(17,901) 13,FTMES(13),"PM=LTT"
      WRITE(17,901) 14,FTMES(14),"PM=LU"
      WRITE(17,901) 15,FTMES(15),"PM=LD"
      WRITE(17,901) 16,FTMES(16),"PM=LT"
      WRITE(17,901) 17,FTMES(17),"PM=LB"
      WRITE(17,901) 18,FTMES(18),"PM=LL"
      WRITE(17,901) 19,FTMES(19),"PM=G1"
      WRITE(17,901) 20,FTMES(20),"PM=G2"
      WRITE(17,901) 21,FTMES(21),"PM=G3"
      WRITE(17,901) NMES+1,FTMES(NMES+1),"MAX"
      WRITE(17,914) NMES+2,INT(FTMES(NMES+2)),"IMAX"
