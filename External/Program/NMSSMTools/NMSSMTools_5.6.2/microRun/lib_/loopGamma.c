...
int loopGamma(double * csAA, double *csAZ)
{
  ...
  *csAA=0,*csAZ=0; 
  ...
  sprintf(buff, LPATH "/microRun/lib/nngg-nm4b/lGamma.exe");
  if(access( buff,X_OK))
  { char buf[2000]; 
    sprintf(buf, "dir=%s/microRun/lib/nngg-nm4b;  which  gmake; if(test $? -eq 0) then  gmake -C $dir; else make -C $dir; fi",LPATH);
    system(buf);
  } 
  if(access( buff,X_OK)) 
  {  
