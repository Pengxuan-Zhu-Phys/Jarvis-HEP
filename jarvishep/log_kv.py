...
def format_two_column_log(
    ...
    *,
    ...
    The formatter guarantees each output line is at most ``max_width`` chars.
    """
    ...
    for key, value in normalized:
        ...
        for idx in range(nline):
            k = key_chunks[idx] if idx < len(key_chunks) else ""
            v = value_chunks[idx] if idx < len(value_chunks) else ""
            lines.append(f"{k:<{key_width}}{separator}{v:<{value_width}}")
