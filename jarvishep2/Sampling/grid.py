...
class Grid(CheckpointedSampler):
    ...
    def initialize(self) -> None:
        t0 = time.time()
        dims = np.array([int(var.parameters["num"]) for var in self.vars], dtype=np.int64)
        self._P = grid_sampling(dims)
    ...
    def propose_next(self) -> Sample | None:
        ...
        while self._index < len(self._P):
            ...
            sample = self._build_sample(np.asarray(row, dtype=np.float64))
