...
from .jarvis_uuid import split_vid_batch, join_v_uid, looks_uuid_augmented
...
def _initialize_live_points(live_points,
    ...
    if live_points is None:
        ...
        live_uid = np.empty(nlive, dtype='U36')  # JARVIS UUID channel
        ...
        while True:
            ...
            cur_live_v = np.array(list(cur_live_v), dtype=object)
            # JARVIS UUID channel: prior_transform may return [...coords, uuid]
            _v_batch, _uid_batch, _full_for_logl = split_vid_batch(cur_live_v)
            if _uid_batch is not None:
                cur_live_v = _v_batch
                cur_live_uid_batch = _uid_batch
                _logl_input = _full_for_logl
            else:
                cur_live_v = np.asarray(cur_live_v, dtype=float)
                cur_live_uid_batch = None
                _logl_input = cur_live_v
            if use_pool_logl:
                cur_live_logl = list(
                    mapper(loglikelihood, np.asarray(_logl_input, dtype=object)))
            else:
                cur_live_logl = list(map(loglikelihood,
                                         np.asarray(_logl_input, dtype=object)))
            if blob:
            ...
            if cur_ngood > 0:
                ...
                if cur_live_uid_batch is not None:  # JARVIS
                    live_uid[ngoods:ngoods + nextra] = cur_live_uid_batch[cur_ind]
                if blob:
        ...
        # JARVIS: optional uid vector at index 2
        if len(live_points) >= 4 and getattr(live_points[2], "dtype", None) is not None \
                and str(getattr(live_points[2].dtype, "kind", "")) == "U":
            live_u, live_v, live_uid, live_logl = live_points[:4]
            if blob and len(live_points) > 4:
                live_blobs = live_points[4]
        else:
            live_u, live_v, live_logl = live_points[:3]
            live_uid = np.array([""] * len(live_u), dtype="U36")
            if blob:
    ...
    if not blob:
        live_blobs = None
    return (live_u, live_v, live_uid, live_logl, live_blobs), logvol_init, ncalls  # JARVIS +uid
...
class Sampler:
    ...
    def __init__(self,
        ...
        self.blob = blob
        # live points
        # JARVIS: live_points may be (u, v, uid, logl, blobs) or stock (u, v, logl, blobs)
        _lp = live_points
        _has_uid = (
            len(_lp) >= 4
            and getattr(_lp[2], "dtype", None) is not None
            and str(getattr(_lp[2].dtype, "kind", "")) == "U"
        )
        if _has_uid:
            self.live_u, self.live_v, self.live_uid, self.live_logl = _lp[:4]
            self.live_blobs = _lp[4] if (blob and len(_lp) > 4) else None
        else:
            self.live_u, self.live_v, self.live_logl = _lp[:3]
            self.live_uid = np.array([""] * len(self.live_u), dtype="U36")
            self.live_blobs = _lp[3] if blob else None
    ...
    def results(self):
        ...
        d = {}
        for k in [
                'nc', 'uid', 'v', 'id', 'it', 'u', 'logwt', 'logl', 'logvol', 'logz',
        ...
        with warnings.catch_warnings():
            ...
            for k in ['id', 'it', 'u', 'uid']:  # JARVIS: samples_uid
    ...
    def add_live_points(self):
        ...
        for i in range(self.nlive):
            ...
            uidstar = self.live_uid[idx].copy()  # JARVIS
            if self.blob:
    ...
    def sample(self,
        ...
        **The main nested sampling loop.** Iteratively replace the worst live
        ...
        for it in range(sys.maxsize):
            ...
            uidstar = self.live_uid[worst].copy()  # JARVIS uuid
            ...
            # JARVIS: strip trailing uuid from prior_transform output if present
            v_arr = np.asarray(v, dtype=object).reshape(-1)
            if looks_uuid_augmented(v_arr):
                uid = str(v_arr[-1])
                v = np.asarray(v_arr[:-1], dtype=float)
            else:
                uid = ''
            ncall += nc
            self.ncall += nc
            ...
            self.live_u[worst] = u
            self.live_v[worst] = v
            self.live_uid[worst] = uid  # JARVIS
