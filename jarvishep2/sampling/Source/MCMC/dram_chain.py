#!/usr/bin/env python3
"""Delayed-Rejection Adaptive Metropolis chain (V1 science, seeded RNG for V2)."""
...
class DRAMChain(AMMCMCChain):
    """DRAM: AM proposal then delayed-rejection stage(s) before final reject."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stage_proposals: dict[int, np.ndarray] = {}
        self._stage_cov: dict[int, np.ndarray] = {}
        self._stage_logl: dict[int, float] = {}
        self._stage_alpha: dict[int, float] = {}

    def _normalize_scale_factors(self, values) -> list[float]:
    ...
    def _clear_stage_cache(self) -> None:
    ...
    def _draw_stage_step(self, stage_index):
        ...
        try:
            return self._rng.multivariate_normal(np.zeros(self._dim, dtype=float), cov)
        except Exception:
    ...
    def _finalize_accept(self, stage_index, accepted_logl) -> None:
    ...
    def _finalize_reject(self, stage_index) -> None:
    ...
    def consume_stage_result(self, stage_index, new_loglikelihood, beta=1.0):
        ...
        if sid == 0:
            ...
            accepted = bool(self._rng.random() < alpha)
            if accepted:
    ...
    def export_state(self) -> dict[str, Any]:
        ...
        base["type"] = "DRAMChain"
        base["dr_steps"] = self._dr_steps
        base["dr_scale_factors"] = list(self._dr_scale_factors)
        base["stage_proposals"] = {
            str(k): np.asarray(v, dtype=float).tolist()
            for k, v in self._stage_proposals.items()
        }
        base["stage_cov"] = {
            str(k): np.asarray(v, dtype=float).tolist() for k, v in self._stage_cov.items()
        }
        base["stage_logl"] = {str(k): float(v) for k, v in self._stage_logl.items()}
        base["stage_alpha"] = {str(k): float(v) for k, v in self._stage_alpha.items()}
        return base
    ...
    def import_state(self, state: dict[str, Any]) -> None:
        ...
        self._stage_proposals = {
            ...
            for k, v in dict(state.get("stage_proposals") or {}).items()
        }
        self._stage_cov = {
            int(k): np.asarray(v, dtype=float)
            for k, v in dict(state.get("stage_cov") or {}).items()
        }
        self._stage_logl = {
            int(k): float(v) for k, v in dict(state.get("stage_logl") or {}).items()
        }
        self._stage_alpha = {
            int(k): float(v) for k, v in dict(state.get("stage_alpha") or {}).items()
        }
