...
D25.5: this process owns the Redis loop, heartbeat, FileOperation lifetime,
and signals. Mapper / layered physics live on :class:`SampleExecutor`.
"""
...
from jarvishep2.calculator_modes import shared_mode_groups
...
from jarvishep2.runtime.sample_executor import SampleExecutor, stamp_task_sample_bucket
...
class Worker(Process):
    ...
    Physics runs on :class:`~jarvishep2.sample_executor.SampleExecutor`.
    """
    ...
    def __init__(
        ...
        self._children_board_open = True
        self._staging_dir = ""
    ...
    def _get_executor(self) -> SampleExecutor:
        ...
        if executor is None:
            self._executor = SampleExecutor(self)
    ...
    def _publish_children_board(self, *, reason: str = "heartbeat") -> None:
        if not self._children_board_open:
            return
        redis = self._redis
        if redis is None:
            return
        ...
        try:
            ...
            try:
                get_jarvis_logger("worker", worker_id=self.worker_id).warning(
                    "children board publish failed -> %s", exc
                )
            except Exception:
                return
    ...
    def _heartbeat(self, status: str | None = None) -> None:
        ...
        if self._children_board_open:
            hb_fields["file_operation_pgid"] = self._file_operation_pgid()
            hb_fields["calc_pgids"] = self._calc_session_leader_pgids()
            hb_fields["children_updated_reason"] = "heartbeat"
        self._redis.heartbeat(str(self.worker_id), **hb_fields)
    ...
    def _run_calculator_step(
        ...
        *,
        ...
        return self._get_executor()._run_calculator_step(step_name, sample, selection_checked=selection_checked)
    ...
    def _force_release_all_held_packs(self, logger: Any | None = None) -> None:
        ...
        return self._get_executor()._force_release_all_held_packs(logger)
    ...
    def _merge_calculator_observables(
        ...
        return self._get_executor()._merge_calculator_observables(sample, step_name, pack_id, updated)

    def _force_serial_layers(self) -> bool:
        """Rollback switch: run same-layer calculators one after another."""
        return self._get_executor()._force_serial_layers()
    ...
    def _run_shared_mode_group(self, steps: list[ExecutionStep], sample: Sample) -> None:
        ...
        return self._get_executor()._run_shared_mode_group(steps, sample)
    ...
    def _run_calculator_steps(self, calc_steps: list[ExecutionStep], sample: Sample) -> None:
        ...
        return self._get_executor()._run_calculator_steps(calc_steps, sample)
