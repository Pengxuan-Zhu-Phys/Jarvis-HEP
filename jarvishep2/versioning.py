#!/usr/bin/env python3
"""Runtime version + logo banner for Jarvis-HEP V2 (V1 visual contract)."""
...
def _render_logo_template_line(line: str, gradient_index: int, gradient_total: int) -> str:
    ...
    text_style = (
        BOLD
        if "Just a Robust" in text or "Author:" in text or "Version:" in text
        else ""
    )
    return (
        f"{_render_icon_cells(cells)}{separator}"
        f"{_ansi_fg(text_color)}{text_style}{text}{RESET}"
    )
...
def _render_logo_template_lines(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        if ICON_TEMPLATE_RE.match(line):
            rendered.append(
                _render_logo_template_line(line, gradient_index, gradient_total)
            )
            gradient_index += 1
        else:
...
def default_logo_path() -> str:
    return os.path.join(os.path.dirname(__file__), "card", "logo")


def render_logo_with_version(logo_path: str | None = None) -> str:
    ...
    # Prefer a V2 product label on the title line when the shared logo template is used.
    for idx, line in enumerate(lines):
        if "Jarvis-HEP" in line and "V2" not in line and ICON_TEMPLATE_RE.match(line):
            lines[idx] = line.replace("Jarvis-HEP", "Jarvis-HEP V2", 1)
            break
