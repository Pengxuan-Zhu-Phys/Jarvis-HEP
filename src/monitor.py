...
class JarvisMonitor:
    ...
    def draw_subprocess_box(self, stdscr, start_y, start_x, height, width):
        ...
        for child in [psutil.Process(self.pid)] + psutil.Process(self.pid).children(recursive=True):
            try:
            ...
            while True:
                ...
                if self.current_view == "Summary":
                    ...
                    if mem < 1024 * 1024 * 1024:
                        ...
                        mem_display = mem / 1024 / 1024 / 1024
                        mem_unit = "GB"
                    ttt = "Processes: {} total; CPU usage: {:.1f}%; Memory usage: {:.1f} {}   {}".format(len(psutil.Process(self.pid).children(recursive=True)), cpu_usage, mem_display, mem_unit, formatted_runtime)                    
                    stdscr.attron(curses.color_pair(2))
                    stdscr.addstr(1, 2, ttt)
                    stdscr.attroff(curses.color_pair(2))
