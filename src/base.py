...
class Base():
    ...
    def manage_directories(self, base_path):
        ...
        file_count = len([name for name in os.listdir(max_dir) if os.path.isfile(os.path.join(max_dir, name))])
        ...
        if file_count > 200:
            ...
            print(f"Directory '{max_number}' contains {file_count} files, no new directory created.")
            return max_dir