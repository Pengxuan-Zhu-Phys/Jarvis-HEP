...
IteratorResult = namedtuple('IteratorResult', [
    'worst', 'ustar', 'vstar', 'uidstar', 'loglstar', 'logvol', 'logwt', 'logz',
    'logzvar', 'h', 'nc', 'worst_it', 'boundidx', 'bounditer', 'eff',
    'delta_logz', 'blob'
])

IteratorResultShort = namedtuple('IteratorResultShort', [
    'worst', 'ustar', 'vstar', 'uidstar', 'loglstar', 'nc', 'worst_it', 'boundidx',
    'bounditer', 'eff'
])
...
class LoglOutput:
    ...
    def __init__(self, v, blob_flag):
        ...
        """
        # print("uilts 74: Call for LoglOutput", v, blob_flag)
        if blob_flag:
            self.val = v[0]
            self.blob = v[1]
        else:
            self.val = v
        # print("uilts 80: LoglOutput >>>", self.val)
...
class LogLikelihood:
    ...
    def __init__(self,
        ...
        # print("utils 149: Init the Likelihood")
        self.uid = None
        self.pool = pool
        self.history_pars = []
        self.history_logl = []
    ...
    def __call__(self, x):
        ...
        """
        # print("utils 191: Call For likelihood", x, "self.loglikelihood", self.loglikelihood.kwargs)
        self.uid = str(uuid4())
        self.loglikelihood.kwargs['uid'] = self.uid
        ret = LoglOutput(self.loglikelihood(x), self.blob)
        # print("utils 197: Call end for Likelihood,    x", x, "uid:    ", self.uid)
        if self.save:
...
_RESULTS_STRUCTURE = [
    ...
    ('samples_uid', 'array[str]', 
     'The unique UUID of the sample XXX (within nlive points)', None),
    ('samples_n', 'array[int]',
...
class Results:
    ...
    def __init__(self, key_values):
        ...
        required_keys = ['samples_u', 'samples_id', 'samples_uid', 'logl', 'samples']
...
def resample_run(res, rstate=None, return_idx=False):
    ...
    new_res_dict = dict(niter=len(res.ncall[samp_idx]),
                        ...
                        samples_uid=res.samples_uid[samp_idx],
                        samples_n=samp_n,
...
def _merge_two(res1, res2, compute_aux=False):
    ...
    for i in range(ntot):
        ...
        for curk in ['id', 'u', 'v', 'uid', 'logl', 'nc', 'it', 'blob']:
    ...
    for curk in ['id', 'it', 'n', 'u', 'uid', 'batch']:
