...
def loglike_cache(x, **uid):
    """
    Likelihood function call
    """
    if uid:
        FunctionCache.logl_kwargs = uid
        # print("pool 40:\tFunction.loglike => {}, args => {}, kwarg => {}".format(FunctionCache.loglike, FunctionCache.logl_args, FunctionCache.logl_kwargs))
    else:
        print("pool 42:\tPool not found the uuid for samples => {} ".format(x))
