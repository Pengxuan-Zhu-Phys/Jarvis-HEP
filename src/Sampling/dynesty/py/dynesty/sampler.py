...
def _generate_uuid_list(x):
    print("Generating the uuid list for: ", x)
    return str(uuid4())
...
class Sampler:
    ...
    live_points : list of 4 `~numpy.ndarray` each with shape (nlive, ndim)
        Initial set of "live" points. 
        Contains 
        `live_u`, the coordinates on the unit cube, 
        `live_v`, the transformed variables, 
        `live_uid`, the uuid for each points,
        and `live_logl`, the associated loglikelihoods.
    ...
    def __init__(self,
        ...
        self.blob = blob
        # live points
        self.live_u, self.live_v, self.live_uid, self.live_logl = live_points[:4]
        if blob:
    ...
    def reset(self):
        ...
        if self.use_pool_ptform:
            ...
            self.live_uid = np.array(
                list(self.M(_generate_uuid_list, np.asarray(self.live_u)))
            )
        else:
            ...
            self.live_uid = np.array(
                list(map(_generate_uuid_list, np.asarray(self.live_u))))
    ...
    def results(self):
        ...
        d = {}
        for k in [
                'nc', 'v', 'id', 'it', 'u', 'uid', 'logwt', 'logl', 'logvol', 'logz',
                'logzvar', 'h', 'blob'
        ]:
    ...
    def _fill_queue(self, loglstar):
        ...
        # print(self._beyond_unit_bound)
        # print("sampler 320: self.method is =>", self.method)
        if self.method != 'unif':
            ...
            args = ()
        # print("sampler 331: args => {}\n\tself._beyond_unit_bound => {}".format(args, self._beyond_unit_bound(loglstar)))
        if self._beyond_unit_bound(loglstar):
            # print("Beyond Unit Bound")
            ...
            # print("sampler 343 Within the unit bound")
        ...
        if self.use_pool_evolve:
            ...
            mapper = map
        args = []
        # print("sampler 369: point_queue =>", point_queue, "seeds =>", seeds)
        for i in range(self.queue_size):
            # print("sampler 371: self.kwargs", self.kwargs.keys(), self.kwargs['update_func'])
            args.append(
        ...
        # print("sampler 379 => Fill queue:", self.queue)

    def _get_point_value(self, loglstar):
        """Grab the first live point proposal in the queue."""

        # If the queue is empty, refill it.
        # print("sampler 384: self.nqueue: ", self.nqueue)
        if self.nqueue <= 0:
            self._fill_queue(loglstar)
        # print("sampler 387: self.nqueue filled")
        # Grab the earliest entry.
        u, v, uid, logl, nc, blob = self.queue.pop(0)
        # print("\tget point value: v: {}, uid: {}".format( v, uid))
        # sleep(3)

        self.used += 1  # add to the total number of used points
        self.nqueue -= 1

        return u, v, uid, logl, nc, blob
    ...
    def _new_point(self, loglstar):
        ...
        ncall, nupdate = 0, 0
        # print("sampler 406: new point")
        while True:
            # Get the next point from the queue
            # u, v, uid, logl, nc, blob = self._get_point_value(loglstar)
            u, v, uid, logl, nc, blob = self._get_point_value(loglstar)
            # print("sampler 411: new point value achieved")
            # print("sampler 412 Get new samples:", v, uid)
            ncall += nc
        ...
        # return u, v, uid, logl, ncall
        # print("sampler 437: new point accept")
        return u, v, uid, logl, ncall
    ...
    def add_live_points(self):
        ...
        for i in range(self.nlive):
            ...
            uidstar= self.live_uid[idx].copy()
            if self.blob:
    ...
    def _remove_live_points(self):
        ...
        if self.added_live:
            ...
            if self.save_samples:
                for k in [
                        'id', 'u', 'v', 'uid', 'logl', 'logvol', 'logwt', 'logz',
    ...
    def sample(self,
        ...
        **The main nested sampling loop.** Iteratively replace the worst live
        ...
        uidstar : `~numpy.ndarray` with shape (ndim,)
            UUID of the sample.
        ...
        """
        # print("save_samples", save_samples)
        # print("saved_run", self.saved_run.__dict__)
        ...
        # print("Sampler.sample logging step 1:", self.it, len(self.saved_run['logl']))
        if self.it == 1 or len(self.saved_run['logl']) == 0:
            # print("Sampler.sample logging step 1: 0 => ", self.it, len(self.saved_run['logl']))
            ...
            if self._beyond_unit_bound(loglstar):
                bound = self.update()
                # print("Sampler.sample logging step 2:")
                if self.save_bounds:
                    self.bound.append(bound)
                    self.nbound += 1
                self.since_update = 0
        else:
            # print("Sampler.sample logging step 1: 1 => ", self.it, len(self.saved_run['logl']))
        ...
        # print(it, sys.maxsize)
        # The main nested sampling loop.
        for it in range(sys.maxsize):
            # print("\nSampler.sample logging step 2 Loops: 0 => \n\tit : {}, sys.maxsize: {}".format(it, sys.maxsize))
            ...
            if dlogz is not None and delta_logz < dlogz:
                stop_iterations = True
            import json 
            # print("\nSampler.sample logging step 2 Loops: 2 => \n\t{}".format(self.__dict__.keys()) )
            # print("Sampler.sample logging step 2 Loops: 3 => \n\t{}".format(self.live_uid) )
            ...
            bcheck = self._beyond_unit_bound(loglstar)
            # print("\nSampler.sample logging step 2 Loops: 3 => \n\t self.since_update : {}, self.update_interval : {}, self._beyond_unit_bound(loglstar) : {}".format(self.since_update, self.update_interval, self._beyond_unit_bound(loglstar)) )
            ...
            # print("\nSampler.sample logging step 2 Loops: 4 => \n\t worst : {}".format(worst) )
            # print("\nSampler.sample logging step 2 Loops: 5 => \n\t self.live_logl : {}".format(self.live_logl) )
            # print("\nSampler.sample logging step 2 Loops: 6 => \n\t argmin : {}".format(np.argmin(self.live_logl)) )
            ...
            uidstar = self.live_uid[worst].copy() # uuid list position
            # print("\tSampler for worstest: ", vstar, uidstar)
            loglstar_new = self.live_logl[worst]  # new likelihood
            # print("\tSampler.sample logging step 2 Loops: 7 => \n\t uuid : {}".format(uidstar) )
            if self.blob:
            ...
            # print("Sampler 823: sample logging step 2 Loops: 7 => \nstar=>\t v: {}, uid: {}, logl: {}".format(vstar, uidstar, loglstar_new) )
            u, v, uid, logl, nc = self._new_point(loglstar_new)
            # print("\tSampler.sample logging step 2 Loops: 7 => \nnew=>\t  v: {}, uid: {}, logl: {}, nc : {}".format(v, uid, logl, nc) )
            # print("Sampler 826: sample logging step 2 Loops: 7 => \nnew=>\t  v: {}, uid: {}, logl: {}, nc : {}".format(v, uid, logl, nc) )
            ncall += nc
            self.ncall += nc
            self.since_update += nc
            # print("\nSampler.sample logging step 2 Loops: 7 => \n\t self.blob : {}".format(self.blob) )
            if self.blob:
                new_blob = logl.blob
            else:
                new_blob = None
            (logwt, logz, logzvar,
             h) = progress_integration(loglstar, loglstar_new, logz, logzvar,
                                       logvol, self.dlv, h)
            # print("\nSampler.sample logging step 2 Loops: 8 => \n\t loglstar : {}, loglstar_new: {}".format(loglstar, loglstar_new) )
            loglstar = loglstar_new
            ...
            # print(self.save_samples)
            # print("\nSampler.sample logging step 2 Loops: 9 => \n\t save_samples : {}".format(self.save_samples) )
            # print("\nSampler.sample logging step 2 Loops: 9 => \n\t saved_run : {}".format(dict(self.saved_run)) )
