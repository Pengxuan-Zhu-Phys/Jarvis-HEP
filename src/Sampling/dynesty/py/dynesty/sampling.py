...
def sample_unif(args):
    ...
    uid : `~numpy.ndarry` with shape (ndim,)
        UUID of the final proposed point in the UUID list.
    ...
    uid = str(uuid4())
    # logl = args.loglikelihood(np.asarray(v), uid)
    # print("sampling 95: Call for likelihood ", v)
    logl = args.loglikelihood(np.asarray(v))
    # print("sampling 97: Call end for likelihood ", v, "loglikelihood.__dict__:", args.loglikelihood.__dict__)
    uid = args.loglikelihood.uid
    # print(logl.__dict__)
    # uid = str(uuid4())
    nc = 1
    blob = None

    return args.u, v, uid, logl, nc, blob
...
def generic_random_walk(u, loglstar, axes, scale, prior_transform,
    ...
    return u, v, uid, logl, ncall, blob
