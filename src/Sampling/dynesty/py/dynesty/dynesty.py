...
class _function_wrapper:
    ...
    def __call__(self, x):
        ...
        try:
            ...
            # print("dynesty 899: {} call for function => {}, inputs => {}".format(self.ncalled, self.func, x, type(x)))
            # print("dynesty 902: {} input => {}".format(len(*args), *args))
            if type(x) == dict:
                y, self.kwargs['uid']  = np.asarray(x['x']).copy(), x['uid']
            else:
                y = np.asarray(x).copy()
            # print("dynasty 902: x => {}, args => {}, kwargs => {}".format(x, self.args, self.kwargs))
            # from time import sleep
            # sleep(4)
            # print("dynesty 902: function.pars", x, self.kwargs)
            return self.func(y, *self.args, **self.kwargs)
        except:  # noqa
            print(f"Exception while {self.ncalled} calling {self.name} function:")
            print("  params:", x)
