...
def _initialize_live_points(live_points,
    ...
    live_points: tuple of arrays or None
        ...
        tuple of 3 arrays (u, v, uid, logl) or
        tuple of 4 arrays (u, v, uid, logl, blobs), i.e.
    ...
    if live_points is None:
        ...
        for _ in range(n_attempts):
            ...
            if use_pool_ptform:
                # print("dynamicsampler 398: pool mapping generating uuid for samples", M)
                live_uid = M(_generate_uuid_list, np.asarray(live_u))
                # print("dynamicsampler 400:", live_uid[0])
                # print("dynamicsampler 401: pool mapping prior_transform")
    ...
    if not blob:
        live_blobs = None
    return (live_u, live_v, live_uid, live_logl, live_blobs)
...
def _configure_batch_sampler(main_sampler,
    ...
    if psel:
        ...
        (live_u, live_v, live_uid, live_logl, live_blobs) = _initialize_live_points(
            None,
            main_sampler.prior_transform,
            main_sampler.loglikelihood,
            main_sampler.M,
        ...
        live_uid = np.empty(nlive_new, dtype=str)
        ...
        for i in range(nlive_new):
            ...
            (live_u[i], live_v[i], live_uid[i], live_logl[i], live_nc[i]) = newpt
            if main_sampler.blob:
                blob = newpt[2].blob
    ...
    batch_sampler.live_u = live_u
    batch_sampler.live_v = live_v
    batch_sampler.live_uid = live_uid
    batch_sampler.live_logl = live_logl
...
class DynamicSampler:
    ...
    def results(self):
        ...
        d = {}
        for k in [
                'nc', 'v', 'uid', 'id', 'batch', 'it', 'u', 'n', 'logwt', 'logl',
    ...
    def sample_initial(self,
        ...
        live_points: list of 4 `~numpy.ndarray` each with shape (nlive, ndim)
            ...
            By default, if these are not provided the initial
        ...
        uidstar : `~numpy.ndarry` with shape (ndim,)
            UUID of the sample
        ...
        if not resume:
            ...
            ii = randint(0, self.nlive_init - 1)
            print("dynamicsampler 1255:", self.live_v.shape, self.live_v[ii], self.live_uid[ii], self.live_logl[ii])
            # print(self.live_v.shape, self.live_v[0])
            # print(self.live_uid.shape, self.live_uid[0])
            print("dynamicsampler 1258: Init the logging: step 1:")
            live_points = [
                self.live_u, self.live_v, self.live_uid, self.live_logl, self.live_blobs
            ]
            self.live_init = [np.array(_) for _ in live_points]
            # print(self.live_init)
            ...
            # print("dynamicsampler 1289: nlive => {}, nlive => {}, results uid => {}".format( self.sampler.nlive, self.sampler.results.nlive, self.sampler.results.samples_uid) )
            import time
            # time.sleep(1)
            # Run the sampler internally as a generator.
            print("Init the logging: step 2:")
        for it, results in enumerate(
            ...
            # print("\ndynamicsampler 1303: nlive => {}, nlive => {}, result => {}, uid => {}, logLikelihood => {}, logweight => {}".format( self.sampler.nlive, self.sampler.results.nlive, self.sampler.results.samples[-1], self.sampler.results.samples_uid[-1], self.sampler.results.logl[-1], self.sampler.results.logwt[-1]) )
            # print("\ndynamicsampler 1304: Init the logging: step 3: in iterloop => ", it, results.vstar, results.uidstar)
            # time.sleep(1)
    ...
    def combine_runs(self):
        ...
        for k in [
                'id', 'u', 'v', 'uid', 'logl', 'nc', 'boundidx', 'it', 'bounditer',
                'n', 'scale', 'blob'
        ]:
        ...
        for _ in range(ntot):
            ...
            for k in [
                    'id', 'u', 'v', 'uid', 'logl', 'nc', 'boundidx', 'it',
