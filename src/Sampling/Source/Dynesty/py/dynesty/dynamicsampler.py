...
def _initialize_live_points(live_points,
    ...
    if live_points is None:
        ...
        live_uid    = np.empty(nlive, dtype='U36')
        # live_uid    = np.zeros(nlive).astype(str)
        live_logl   = np.zeros(nlive)
        ngoods = 0  # counter for how many finite logl we have found
        live_blobs = []
        iattempt = 0
        while True:
            # print("Line 430 ->", live_uid[0])
            iattempt += 1

            # simulate nlive points by uniform sampling
            cur_live_u = rstate.random(size=(nlive, npdim))
            if use_pool_ptform:
                # print(f"Line 434 @ dynamicsampler -> {use_pool_ptform}")
                cur_live_vid = M(prior_transform, np.asarray(cur_live_u))
            else:
                cur_live_vid = map(prior_transform, np.asarray(cur_live_u))
            cur_live_vid        = np.array(list(cur_live_vid))
            ...
            if cur_ngood > 0:
                ...
                # print(len(cur_ind), len(live_uid), len(live_uid))
                # print(ngoods, nextra, live_uid[0])
            ...
            if not np.isfinite(logl):
                if np.sign(logl) < 0:
                    ...
                    raise ValueError("The log-likelihood ({0}) of live "
                                     ...
                                     " is invalid.".format(
                                         logl, i, live_u[i], live_v[i], live_uid[i]))
    ...
    if not blob:
        live_blobs = None
    return (live_u, live_v, live_uid, live_logl, live_blobs), logvol_init, ncalls
...
def _configure_batch_sampler(main_sampler,
    ...
    first_points = []
    # print("\ndynamicsampler Line 622 ->", main_sampler.live_init[2][0])
    ...
    if psel:
        ...
        for i in range(nlive_new):
            # print("Line 686 ->", live_uid[i])
            # TODO is the self.eff the right efficiency to use here
            # check worst_it
            # print("dynamicsampler Line 692 ->", live_uid[i])
            first_points.append(
        ...
        for i in range(nlive_new):
            ...
            # print("Line 800 -> live_u", newpt[0])
            # print("Line 800 -> live_v", newpt[1][:-1].astype(float))
            # print("dynamicsampler Line 800 -> live_uid", newpt[1][-1])
            # print("Line 800 -> live_logl", newpt[2])
            # print("Line 800 -> live_nc", newpt[3])
            live_u[i] = newpt[0]
            live_v[i] = newpt[1][:-1].astype(float)
            live_uid[i] = newpt[1][-1]
            live_logl[i] = newpt[2]
            live_nc[i] = newpt[3] 
            
            # print("dynamicsampler Line 832 ->", newpt[1][-1], live_uid)
...
class DynamicSampler:
    ...
    def results(self):
        ...
        # print()
        for k in [
                'nc', 'uid', 'v', 'id', 'batch', 'it', 'u', 'n', 'logwt', 'logl',
                'logvol', 'logz', 'logzvar', 'h', 'batch_nlive',
                'batch_bounds', 'blob'
        ]:
            d[k] = np.array(self.saved_run[k])
        # print("dynamicsampler Line 1110 ->", self.saved_run)
        ...
        # print("dynamicsampler Line 1136 ->", Results._ALLOWED)
        # print("dynamicsampler Line 1137 ->", results)
    ...
    def sample_initial(self,
        ...
        if not resume:
            ...
            # print("dynamicsampler Line 1331 ->, ", self.live_uid[0], self.live_v[0])
            live_points = [
            ...
            # print("dynamicsampler Line 1339 ->", self.live_init[0][0], self.live_init[1][0], self.live_init[2][0], self.bounding)
            ...
            # print("dynamicsampler Line 1362 ->", self.sampler.__dict__.keys(), type(self.sampler))
        for it, results in enumerate(
            ...
            # print("dynestysampler Line 1404 ->", add_info['uid'])
        ...
        for it, results in enumerate(self.sampler.add_live_points()):
            ...
            add_info = dict(id=results.worst,
                            ...
                            bounditer=results.bounditer,
                            scale=self.sampler.scale)
            # print("dynestysampler Line 1451 ->", add_info['uid'])
            self.base_run.append(add_info)
            self.saved_run.append(add_info)
    ...
    def sample_batch(self,
        ...
        for _ in range(len(batch_sampler.first_points)):
            # print("dynamicsampler Line 1631 ->", batch_sampler.first_points[0])
        ...
        for it, results in enumerate(
            ...
            # if len(D['uid']) == 1:
            #     print("dynamicsampler Line 1662 ->", D['uid'])
        ...
        for it, results in enumerate(batch_sampler.add_live_points()):
            ...
            D = dict(id=results.worst,
                     ...
                     bounditer=results.bounditer,
                     scale=batch_sampler.scale)
            # print("dynamicsampler Line 1714 ->", D['uid'])
            self.new_run.append(D)
        ...
        if maxcall > 0 and maxiter > 0:
            ...
            try:
                ...
                for cur_results in self.sample_batch(nlive_new=nlive,
                    ...
                    # print("dynamicsampler Line 2350 ->", cur_results.uidstar)
                    # Reorganize results.
