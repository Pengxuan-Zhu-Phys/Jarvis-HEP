...
class reader():
    ...
    def set_value(self):
        ...
        self.record_pDK_result()
        self.data['data'].to_csv(self.datacsv, index=False)

    def record_pDK_result(self):
        def record_fsts_info(var):
            if "json" in self.pdkoutput:
                var['jspath'] = os.path.join(os.path.dirname(self.parser), "{}DKs.json".format(var['code']))
                self.logfile.write("\n\t{}\tDecay Information save to\n\t\t\tInfo ->\t\t {}\n\t\t\tData ->\t\t {}\n".format(var['code'], var['jspath'], var['csvpath']))
                import json
                with open(var['jspath'], "w") as f1:
                    json.dump(var['fsts'], f1, indent = 4, sort_keys=True)

        for var in self.pDK:
            var['data'] = pd.DataFrame(var['data'])
            var['data'] = var['data'].fillna(0.)
        if self.cf.has_option("Info", "pDKoutput"):
            self.pdkoutput = self.cf.get("Info", "pDKoutput")
            if "True" in self.pdkoutput:
                self.logfile.write("\nParticle Decay Info\n")
                for var in self.pDK:
                    var['csvpath'] = os.path.join(os.path.dirname(self.parser), "{}DKs.csv".format(var['code']))
                    var['data'].to_csv(var['csvpath'], index=False)
                    record_fsts_info(var)

        # print(self.pDK[0]['data'], len(self.pDK[0]['fsts']))
    ...
    def get_data_from_files(self, data, files):
        ...
        def get_pDK(spectr, readlib, var):
            if readlib == 'PySLHA':
                brls = []
                for item in spectr.decays[var['code']].decays:
                    if tuple(item.ids) not in var['brls']:
                        var['brls'].append(tuple(item.ids))
                        brls.append(tuple(item.ids))
                if brls:
                    self.collect_final_state(var, brls)
                ld = {}
                ld['Index'] = data['Index']
                from itertools import permutations
                for item in var['fsts']:
                    br = 0. 
                    try:
                        for its in spectr.decays[var['code']].decays:
                            for fstate in item['finalstates']:
                                if tuple(its.ids) in permutations(fstate):
                                    br += its.br
                    except:
                        pass
                    ld[item['name']] = br
                var['data'].append(pd.Series(ld))

            elif readlib == 'xSLHA':
                # print(spectr.br[var['code']].items())
                brls = []
                for ff, vv in spectr.br[var['code']].items():
                    if ff not in var['brls']:
                        var['brls'].append(tuple(ff))
                        brls.append(tuple(ff))
                if brls:
                    self.collect_final_state(var, brls)
                ld = {}
                ld['Index'] = data['Index']
                ...
                for item in var['fsts']:
                    ...
                    for fstate in item['finalstates']:
                        try:
                            br += spectr.Value("BR", [var['code'], fstate])
                        except:
                            pass 
                    ld[item['name']] = br 
                var['data'].append(pd.Series(ld))

        def get_pDK_NTools(spectr, var):
            brls = []
            spectr("WIDTH", var['code'])
            fsts = self.pdkspectr['data'][var['code']]
            for it in fsts:
                if type(it) == tuple:
                    if it not in var['brls']:
                        var['brls'].append(it)
                        brls.append(it)
            if brls:
                self.collect_final_state(var, brls)
            ...
            for item in var['fsts']:
                br = 0. 
                for fstate in item['finalstates']:
                    if fstate in fsts.keys():
                        vls = spectr("DECAY", var['code'], fstate)
                        br += vls
                ld[item['name']] = br 
            var['data'].append(pd.Series(ld))
        ...
        if self.task['format'] == "Standard SLHA":
            ...
            if "spectr" in files.keys():
                ...
                if self.ispDK:
                    spectr("WIDTH", 1000023)
                    self.pdkspectr = dict(vars(vars(spectr)["DECAY"]))
                    for var in self.pDK:
                        get_pDK_NTools(spectr, var)
        return data 

    def collect_final_state(self, var, brls):
        mp = str(var['code'])
        for fs in brls:
            nn = []
            for pp in self.particle:
                ...
                for ii in range(len(fs)):
                    ...
                    if sp[0] == '-' and sp[1:] == pp['PDG_code']:
                        ...
                        nn.append(sp)       
            fst = {
                "name": "{}dk2{}".format(mp, "".join(nn)),
                "finalstates":  fs
            }
            # print("{}dk2{}".format(mp, "".join(nn)), tuple([-x for x in fs]), fst)   
            if var['meth'] == "All":
                from itertools import permutations, combinations
                isInTag = False
                for ff in var['fsts']:
                    for fs in ff['finalstates']:
                        # print( fst['finalstates'] in permutations(fs), fst['finalstates'], list(permutations(fs)))
                        if fst['finalstates'] in permutations(fs):
                            isInTag = True
                        elif (fst['finalstates'] in permutations(tuple([-x for x in fs]))) and (fst['finalstates'] not in permutations(fs)):
                            isInTag = True
                            isPermu = False
                            for ids in permutations(fst['finalstates']):
                                if ids in ff['finalstates']:
                                    isPermu = True
                            if not isPermu:
                                ff['finalstates'].append(fst['finalstates'])                        
                if not isInTag:
                    var['fsts'].append({
                        "name": fst['name'],
                        "finalstates":  [fst['finalstates']]
                    })
    ...
    def get_variables(self):
        ...
        if self.task['format'] == "NMSSMTools":
            ...
            for var in variable:
                ...
                if varinfo[1].strip().upper() == "SLHA" or varinfo[1].strip() == "FindSLHA" or varinfo[1].strip() == "FindBR" or varinfo[1].strip() == "FindDK":
                    # print("Info -> Using default name for variable:\n\t{}".format(var))
                    pass 
                    # print()
                elif varinfo[2].strip().upper() == "SLHA" or varinfo[2].strip() == "FindSLHA" or varinfo[2].strip() == "FindBR" or varinfo[2].strip() == "FindDK":
                    dfname = 1
                else:
                ...
                elif varinfo[1+dfname].strip() == "FindDK":
                    self.pDK.append({
                        "code":     int(varinfo[2+dfname].strip()),
                        "meth":     self.get_FindDK_mode(varinfo[2+dfname:]),
                        "patt":     varinfo[0].strip(),
                        "data":     [],
                        "fsts":     [],
                        "brls":     [],
        ...
        if self.task['format'] == "Standard SLHA":
            ...
            for var in variable:
                ...
                if varinfo[0].strip() not in self.task['pattern'].keys():
                    print("Error -> pattern '{}' not founded for variable".format(varinfo[0], var))
                    sys.exit(0)
                if varinfo[1].strip() == "SLHA" or varinfo[1].strip() == "FindSLHA" or varinfo[1].strip() == "FindBR" or varinfo[1].strip() == "FindDK":
                    # print("Info -> Using default name for variable:\n\t{}".format(var))
                    pass 
                elif varinfo[2].strip() == "SLHA" or varinfo[2].strip() == "FindSLHA" or varinfo[2].strip() == "FindBR" or varinfo[2].strip() == "FindDK":
                    dfname = 1
                else:
                    print("Error -> Only 'SLHA', 'FindBR', 'FindDK' and 'FindSLHA' method support in this version")
                    sys.exit(0)
                if varinfo[1+dfname].strip() == "SLHA":
                    if varinfo[2+dfname].strip().upper() == "BLOCK":
                        if len(varinfo[3+dfname:]) == 2:
