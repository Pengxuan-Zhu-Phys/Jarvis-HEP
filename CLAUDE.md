...
- **M1 acceptance gate** (WP-1.4): `tests/test_output_parity.py` locks calculator DATABASE/SAMPLE parity (`tests/parity_project`, `--check-modules` scale) and opera DATABASE contract under `sample_artifacts: auto|always`. M1 throughput gate: ≥ 2.5× the local WP-0.1 baseline (or ≥ 2,000 samples/s on the reference machine). Calculator `@Sdir` IO paths are left as tokens through `analysis_calculator()` so artifacts resolve under the per-sample save directory.
